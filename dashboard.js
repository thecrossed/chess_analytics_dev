...
function renderSummary(payload) {
  ...
  const nonEmptyPoints = payload.points.filter((point) => point.games > 0 && typeof point.win_rate === "number");
  ...
  summaryGridEl.append(
    ...
    buildSummaryCard(
      ...
      formatPercent(payload.overall_win_rate),
      t("dashboard_summary_overall_detail")
    ),
    buildSummaryCard(
...
function renderChart(payload) {
  ...
  chartEl.appendChild(svgEl("rect", { x: 0, y: 0, width, height, fill: "#faf8f2" }));

  [0, 0.25, 0.5, 0.75, 1].forEach((tick) => {
    const y = yForValue(tick);
    chartEl.appendChild(svgEl("line", { x1: margin.left, y1: y, x2: width - margin.right, y2: y, stroke: tick === 0.5 ? "#cdbfaf" : "#ded6ca", "stroke-width": 1 }));
  ...
  chartEl.appendChild(svgEl("line", { x1: margin.left, y1: xAxisY, x2: width - margin.right, y2: xAxisY, stroke: "#b7aea0", "stroke-width": 1 }));
  ...
  points.forEach((point, index) => {
    ...
    chartEl.appendChild(svgEl("line", { x1: x, y1: margin.top, x2: x, y2: xAxisY, stroke: "#efe8de", "stroke-width": 1 }));
    const label = svgEl("text", {
      x,
  ...
  if (typeof lastIndex === "number") {
    ...
    const y = yForValue(point.win_rate);
    chartEl.appendChild(svgEl("circle", { cx: x, cy: y, r: 5.5, fill: "#d13c32", stroke: "#faf8f2", "stroke-width": 2 }));

    const calloutLineY = Math.max(y - 42, margin.top + 18);
    chartEl.appendChild(svgEl("line", { x1: x, y1: y - 6, x2: x, y2: calloutLineY, stroke: "#d13c32", "stroke-width": 1.5 }));
...
async function loadDashboard() {
  try {
    ...
    renderChart(payload);
    if (chartNoteEl) {
      chartNoteEl.textContent = `${t("dashboard_chart_source")}: Chess.com PubAPI • ${payload.window_label} • ${formatInteger(payload.total_games)} ${t("dashboard_games_suffix")}`;
    }
...
window.addEventListener("languagechange", () => {
  ...
  if (chartNoteEl) {
    chartNoteEl.textContent = `${t("dashboard_chart_source")}: Chess.com PubAPI • ${dashboardPayload.window_label} • ${formatInteger(dashboardPayload.total_games)} ${t("dashboard_games_suffix")}`;
  }
});
